...
class Coord {
    ...
    method get_dir ($dir) {
        ...
        if ( $dir eq '^' )  { return $self->n(); }
        if ( $dir eq 'v' )  { return $self->s(); }
        if ( $dir eq '>' )  { return $self->e(); }
        if ( $dir eq '<' )  { return $self->w(); }
    ...
    method equal($c) { $c->row == $row and $c->col == $col }

    method dist($c) { abs($c->row - $row) + abs($c->col - $col) }
